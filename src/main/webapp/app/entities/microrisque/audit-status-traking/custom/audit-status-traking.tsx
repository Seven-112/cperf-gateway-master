...
const TrakItem = (props: {trak: IAuditStatusTraking,  account: any, handleDelete?: Function}) =>{
    ...
    const getUser = () =>{
        if(trak && trak.userId){
            setLoadingUser(true)
            axios.get<IUserExtra>(`${API_URIS.userExtraApiUri}/${trak.userId}`)
                .then(res =>{
                    if(res.data)
    ...
    const getFiles = () =>{
        if(trak && trak.id){
            ...
            getMshzFileByEntityIdAndEntityTag(trak.id, FileEntityTag.auditStatusTraking)
                .then(res =>{
                    console.log(res)
                    if(res.data)
                     setFiles([...res.data]);
    ...
    return (
        <React.Fragment>
            ...
            {trak && <>
                <TableRow>
                    <TableCell>{trak.status ? translate(`_global.TaskAction.${trak.status.toString()}`) : '...'}</TableCell>
...
interface AuditStatusTrakingProps{
    open?:boolean,
    entity:IAudit | IAuditRecommendation,
    isRecom?: boolean,
    account: any,
    userAuditRoles: AuditUserRole[],
    onClose:Function,
}

export const AuditStatusTraking = (props: AuditStatusTrakingProps) =>{
    const { open, entity, account, userAuditRoles, isRecom } = props;
    const [loading, setLoading] = useState(false);
    const [tracks, setTracks] = useState<IAuditStatusTraking[]>([]);
    const [activePage, setActivePage] = useState(0);
    const [itemsPerPage, setItemsPerPage] = useState(10);
    const [totalItems, setTotalItems] = useState(0);
    const [trackToUpdate,setTrackToUpdate] = useState<IAuditStatusTraking>(null);
    const [openToUpdate, setOpenToUpdate] = useState(false);
    const [openToDel, setOpenToDel] = useState(false);

    const classes = useStyles();

    const userIsSubmitor = [...userAuditRoles].find(role => role === AuditUserRole.SUBMITOR) ? true : false;
    const userIsValidator = [...userAuditRoles].find(role => role === AuditUserRole.VALIDATOR) ? true : false;
    const userIsExecutor = [...userAuditRoles].find(role => role === AuditUserRole.EXECUTOR) ? true : false;

    const hasAddPrivileges = account && hasPrivileges({ entities: ['Audit'], actions: [PrivilegeAction.CREATE]}, account.authorities)
    const hasUpdatePrivileges = account && hasPrivileges({ entities: ['Audit'], actions: [PrivilegeAction.UPDATE] }, account.authorities);
    const handDeletePrivilges = account && hasPrivileges({ entities: ['Audit'], actions: [PrivilegeAction.DELETE] }, account.authorities);
    const handViewPrivilges = account && hasPrivileges({ entities: ['Audit'], actions: [PrivilegeAction.ALL] }, account.authorities);

    const canAdd = entity && (hasAddPrivileges 
          || entity.status === AuditStatus.STARTED && userIsExecutor 
          || entity.status === AuditStatus.EXECUTED && userIsSubmitor 
          || entity.status === AuditStatus.SUBMITTED && userIsValidator);

    const getTracks = (p?: number) =>{
        const page = p || p === 0 ? p : activePage;
        if(props.entity && props.entity.id){
            let requestUri = `${API_URIS.auditStatusTrackingApiUri}/?auditId.equals=${props.entity.id}`;
            requestUri = `${requestUri}&page=${page}&size=${itemsPerPage}&sort=id,desc`;
            requestUri = `${requestUri}&recom.specified=${isRecom ? 'true': 'false'}`
            setLoading(true)
            axios.get<IAuditStatusTraking[]>(requestUri)
                .then(res =>{
                    if(res.data){
                        setTracks(res.data);
    ...
    const handleDelete = (track: IAuditStatusTraking) =>{
        setTrackToUpdate(track);
        setOpenToDel(true);
    }

    const items = [...tracks].map((track, index) =>(
            <TrakItem key={index} trak={track}
             account={account} handleDelete={handleDelete} />));
    ...
    const handleUpdate = (tTrack?: IAuditStatusTraking) =>{
        setTrackToUpdate(tTrack);
        setOpenToUpdate(true);
    }

    const onSave = (saved?: IAuditStatusTraking, isNew?: boolean) =>{
        if(saved){
            if(isNew)
        ...
        <React.Fragment>
            ...
            <MyCustomModal
               ...
               customActionButtons={canAdd ? (
                ...
                title={translate("microgatewayApp.microrisqueAuditStatusTraking.home.title")}
                footer={ totalItems > 0 ? 
            ...
            {entity && <> 
                ...
                {/* <Box width={1} textAlign="center" 
                    ...
                    textOverflow="text-wrap" p={1}>
                        <Typography variant="h4">
                            {`${translate("microgatewayApp.microrisqueAudit.detail.title")}`}
                        </Typography>
                </Box> */}
                <Box width={1} mt={2} mb={2} p={2} pt={1}>
                    <Table>
                        <TableHead>
                            <TableRow>
                                <TableCell align="center">{translate("microgatewayApp.microrisqueAuditStatusTraking.status")}</TableCell>
                                <TableCell align="center">{translate("microgatewayApp.microrisqueAuditStatusTraking.tracingAt")}</TableCell>
                                <TableCell align="center">{translate("microgatewayApp.microrisqueAuditStatusTraking.userId")}</TableCell>
                                <TableCell align="center">{translate("microgatewayApp.microrisqueAuditStatusTraking.justification")}</TableCell>
                                ...
                                <TableRow>
                                    <TableCell align="center" colSpan={10}>
                                        <Box width={1} display="flex" justifyContent="center" alignItems="center"
                                            flexWrap="wrap" overflow="auto" p={2}>
                                            <Typography>
                                                {!isRecom ? translate("microgatewayApp.microrisqueAudit.home.notFound") 
                                                    : translate("microgatewayApp.microrisqueAuditRecommendation.home.notFound")}
                                            </Typography>
                                        </Box>
                                    </TableCell>
                                </TableRow>}
...
export default AuditStatusTraking;
