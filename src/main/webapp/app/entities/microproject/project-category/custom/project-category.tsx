...
export const ProjectCategoryUpdate = (props: EvaluationUpdateProps) =>{
    const { open } = props;
    ...
    const handleSave = (event) =>{
        ...
        if(category && category.name){
            const request = isNew ? axios.post<IProjectCategory>(`${API_URIS.projectCategoryApiUri}`, cleanEntity(category))
                    : axios.put<IProjectCategory>(`${API_URIS.projectCategoryApiUri}`, cleanEntity(category))
            request.then(res =>{
                if(res.data){
                    setSuccess(true)
                    if(props.onSave){
                    ...
                    <CardHeader 
                        title={<Box display="flex" flexWrap="wrap" overflow="hidden">
                                <Typography variant="h4" className="mr-5">
                                    {translate("microgatewayApp.microprojectProjectCategory.home.createOrEditLabel")}
                                </Typography>
                        </Box>}
                            ...
                            <Grid container spacing={2}>
                                ...
                                <Grid item xs={12}>
                                    <TextField 
                                        ...
                                        name="name"
                                        size="small"
                                        label={translate("microgatewayApp.microprojectProjectCategory.name")}
