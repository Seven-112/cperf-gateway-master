...
const ItemCheckJustificationRow = (props: {
    ...
    const date = entity && entity.dateAndTime ? new Date(entity.dateAndTime) : null;
...
interface IProjectItemCheckJustificationProps{
    taskItem: IProjectTaskItem,
    readonly?:boolean,
    open?: boolean,
    onClose: Function,
}
...
export const ProjectItemCheckJustification = (props: IProjectItemCheckJustificationProps) =>{
    ...
    const [justifications, setJustifications] = useState<IProjectItemCheckJustification[]>([]);
    const [selectedJustification, setSelectedJustification] = useState<IProjectItemCheckJustification>(null);
    const [currentJustification, setCurrentJustification] = useState<IProjectItemCheckJustification>(null);
    ...
    const getCurrenJustification = () =>{
        if(props.taskItem){
            setLoading(true);
            let apiUri = `${API_URIS.projectTaskItemCheckJustificationApiUri}/?taskItemId.equals=${props.taskItem.id}`;
            apiUri = `${apiUri}&checked.equals=${taskItem.checked}&page=${0}&size=${1}&sort=id,desc`;

            axios.get<IProjectItemCheckJustification[]>(apiUri)
                .then(res => {
    ...
    const handleUpdate = (justification?: IProjectItemCheckJustification) =>{
    ...
    const handleDelete = (jst?:IProjectItemCheckJustification) =>{
        if(jst){
    ...
    const onSave = (saved?: IProjectItemCheckJustification, isNew?:boolean) =>{
        if(saved){
            if(isNew){
    ...
    return (
        ...
        {selectedJustification && !readonly && <>
            <ProjectItemCheckJustificationUpdate 
                open={openToUpdate}
            ...
            <EntityDeleterModal 
                ...
                urlWithoutEntityId={API_URIS.projectTaskItemCheckJustificationApiUri}
                onDelete={onDelete}
  ...
  export default ProjectItemCheckJustification;
