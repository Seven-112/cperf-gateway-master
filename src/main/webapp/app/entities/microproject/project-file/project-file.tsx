...
export const ProjectFile = (props: IProjectFileProps) => {
  ...
  const { projectFileList, match, loading, totalItems } = props;
  return (
    <div>
      <h2 id="project-file-heading">
        <Translate contentKey="microgatewayApp.microprojectProjectFile.home.title">Project Files</Translate>
        <Link to={`${match.url}/new`} className="btn btn-primary float-right jh-create-entity" id="jh-create-entity">
          <FontAwesomeIcon icon="plus" />
          &nbsp;
          <Translate contentKey="microgatewayApp.microprojectProjectFile.home.createLabel">Create new Project File</Translate>
        </Link>
      </h2>
      <div className="table-responsive">
        {projectFileList && projectFileList.length > 0 ? (
          <Table responsive>
            <thead>
              <tr>
                ...
                <th className="hand" onClick={sort('fileId')}>
                  <Translate contentKey="microgatewayApp.microprojectProjectFile.fileId">File Id</Translate> <FontAwesomeIcon icon="sort" />
                </th>
                ...
                <th className="hand" onClick={sort('userId')}>
                  <Translate contentKey="microgatewayApp.microprojectProjectFile.userId">User Id</Translate> <FontAwesomeIcon icon="sort" />
                </th>
            ...
            <tbody>
              {projectFileList.map((projectFile, i) => (
                <tr key={`entity-${i}`}>
                  ...
                  <td>
                    <Translate contentKey={`microgatewayApp.ProjectFileType.${projectFile.fileType}`} />
                  </td>
                  <td>{projectFile.projectId}</td>
                  <td>{projectFile.userId}</td>
                  <td>{projectFile.userName}</td>
                  <td>{projectFile.userEmail}</td>
                  <td className="text-right">
                    <div className="btn-group flex-btn-group-container">
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${projectFile.id}/edit?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="primary"
                        size="sm"
                      >
                      ...
                      <Button
                        tag={Link}
                        to={`${match.url}/${projectFile.id}/delete?page=${paginationState.activePage}&sort=${paginationState.sort},${paginationState.order}`}
                        color="danger"
                        size="sm"
                      >
          ...
          !loading && (
            <div className="alert alert-warning">
              <Translate contentKey="microgatewayApp.microprojectProjectFile.home.notFound">No Project Files found</Translate>
            </div>
          )
        )}
      </div>
      {props.totalItems ? (
        <div className={projectFileList && projectFileList.length > 0 ? '' : 'd-none'}>
