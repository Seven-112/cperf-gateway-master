...
const EventTriggerRow = (props: {event?: IEventTrigger, onUpdate?:Function, onDelete?: Function}) =>{
    const { event } = props;
...
export const EventTrigger = (props: EventTriggerProps) =>{
    const { open } = props;
    const [userExtra, setUserExtra] = useState<IUserExtra>(null);
    const [entity, setEntity] = useState<IProcess>(props.process);
    const [events, setEvents] = useState<IEventTrigger[]>([]);
    const [selectedEvent, setSelectedEvent] = useState<IEventTrigger>(null);
    ...
    const getEvents = (p?: number, rows?:number) =>{
        if(entity){
            ...
            let apiUri = `${API_URIS.processEventApiUri}/?processId.equals=${entity.id}`;
            apiUri = `${apiUri}&page=${page}&size=${size}&sort=id,desc`;

            axios.get<IEventTrigger[]>(apiUri)
                .then(res => {
                    setEvents([...res.data])
    ...
    const handleUpdate = (ev?: IEventTrigger) =>{
        if(ev){
            setSelectedEvent(ev);
        }else{
            const date = new Date();
            setSelectedEvent({
                process: entity, 
                recurrence: ProcessEventRecurrence.ONCE,
    ...
    const handleDelete = (ev?:IEventTrigger) =>{
        if(ev){
            setSelectedEvent(ev);
    ...
    const onSave = (saved?: IEventTrigger, isNew?:boolean) =>{
        if(saved){
            if(isNew)
    ...
    const items = [...events].map((e, index) =>(
        <EventTriggerRow key={index} event={e} onUpdate={handleUpdate} onDelete={handleDelete}/>
    ))
    ...
    return (
        ...
        <EntityDeleterModal 
            ...
            urlWithoutEntityId={API_URIS.processEventApiUri}
            onDelete={onDelete}
