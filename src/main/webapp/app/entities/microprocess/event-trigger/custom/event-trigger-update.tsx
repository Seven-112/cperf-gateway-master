...
export const AgendaEventUpdate = (props: AgendaEventUpdateProps) =>{
    ...
    const formIsValid = entity && entity.name && entity.process && entity.nextStartAt;
    ...
    const handleSubmit = (event) =>{
        ...
        if(formIsValid){
            ...
            setLoading(true);
            const req = isNew ? axios.post<IEventTrigger>(API_URIS.processEventApiUri, cleanEntity(entity))
                            : axios.put<IEventTrigger>(`${API_URIS.processEventApiUri}`, cleanEntity(entity));
            req.then(res =>{
                if(res.data){
                    setEntity(res.data);
                    if(props.onSave)
                                    ...
                                    <Box width={1} display="flex" justifyItems="center"
                                        ...
                                        <Select 
                                            ...
                                            <MenuItem value={EventRecurrence.ONCE}>
                                                {getEventOccurenceLabel(dateTime, EventRecurrence.ONCE)}
                                            </MenuItem>
                                            <MenuItem value={EventRecurrence.ALLAWAYS}>
                                                {getEventOccurenceLabel(dateTime,EventRecurrence.ALLAWAYS)}
                                            </MenuItem>
                                            <MenuItem value={EventRecurrence.EVERY_WEEK_ON_DAY}>
                                                {getEventOccurenceLabel(dateTime, EventRecurrence.EVERY_WEEK_ON_DAY)}
                                            </MenuItem>
                                            <MenuItem value={EventRecurrence.EVERY_MONTH}>
                                                {getEventOccurenceLabel(dateTime, EventRecurrence.EVERY_MONTH)}
                                            </MenuItem>
                                           ...
                                           {/*  <MenuItem value={EventRecurrence.WEEK}>
                                                {getEventOccurenceLabel(dateTime, EventRecurrence.WEEK)}
                                            </MenuItem>
