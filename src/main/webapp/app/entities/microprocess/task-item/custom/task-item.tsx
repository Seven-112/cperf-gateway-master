...
import { Backdrop, Box, Card, CardActions, CardContent, CardHeader, Checkbox, Chip, CircularProgress, IconButton, List, ListItem, ListItemIcon, ListItemSecondaryAction, ListItemText, makeStyles, Modal, Slide, TablePagination, Typography } from "@material-ui/core";
...
import ItemCheckJustification from "../../item-check-justification/custom/cheick-item-justification";
...
const TaskItemListItem = (props: {
        item?: ITaskItem,editable?:boolean,
        cheickable?:boolean, taskInInstance?:boolean,
        handleShowJustification?: Function,
        onCheickchange?:Function, onUpdate?:Function, onDelete?: Function
    }) =>{
    const {editable, cheickable, taskInInstance } = props;
                ...
                <ListItem className="w-100" button>
                    ...
                    <ListItemSecondaryAction>
                        <Box display="flex" alignItems="center" flexWrap="wrap">
                            {(props.handleShowJustification && taskInInstance) &&
...
export const TaskItem = (props: TaskItemProps) =>{
    ...
    const [activeItem, setActiveItem] = useState<ITaskItem>(null);
    ...
    const [taskProcess, setTaskProcess] = useState<IProcess>(null);
    ...
    const readonly = !task || !account || !taskProcess || (taskProcess && taskProcess.canceledAt)
                     || (task.status === TaskStatus.COMPLETED || task.status === TaskStatus.CANCELED)
                     || !hasPrivileges({entities: ['Process', 'Task'], actions: [PrivilegeAction.ALL]}, account.authorities);

    const taskInInstance= (taskProcess && taskProcess.modelId) ? true : false;
    
    const getProcess = () =>{
        if(props.task && props.task.processId){
            axios.get<IProcess>(`${API_URIS.processApiUri}/${props.task.processId}`)
                .then(res =>{
                    setTaskProcess(res.data);
                }).catch(e => {
                    console.log(e)
                    setTaskProcess(null);
                })
    ...
    const getTaskItems = (p?: number, rows?:number) =>{
        if(props.task){
            ...
            let apiUri = `${API_URIS.taskItemApiUri}/?taskId.equals=${props.task.id}`;
            apiUri = `${apiUri}&page=${page}&size=${size}&sort=id,desc`;

            axios.get<ITaskItem[]>(apiUri)
                .then(res => {
    ...
    const handleUpdate = (it?: ITaskItem) =>{
        if(!readonly){
            if(it){
                setActiveItem(it);
            }else{
    ...
    const handleDelete = (it?:ITaskItem) =>{
        if(it && !readonly){
            setActiveItem(it);
    ...
    const handleShowJustification = (it?:ITaskItem) =>{
        if(it){
            setActiveItem(it);
    ...
    const onSave = (saved?: ITaskItem, isNew?:boolean) =>{
        if(saved){
            if(isNew)
    ...
    const onCheickChange = (item: ITaskItem) =>{
        if(item && item.id){
            const entity: ITaskItem ={
                ...item,
                checked:!item.checked
            }
            setLoading(true);
            axios.put<ITaskItem>(API_URIS.taskItemApiUri, cleanEntity(entity))
            .then(res =>{
                if(res.data){
                    if(props.onCheckChange)
                        props.onCheckChange();
    ...
    const itemIsCheickable = (ti: ITaskItem) =>{
        return (ti && userExtra && task && userExtra.id === ti.checkerId);
    }

    const items = [...taskItems].map((item, index) =>(
        <TaskItemListItem key={index} 
        item={item} taskInInstance={taskInInstance}
