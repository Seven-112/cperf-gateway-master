...
import TaskUserTab from "./task-user-tab";
...
export const TaskUserModal = (props: ITaskUserModalProps) =>{
    const [tabs, setTabs] = useState<ITab[]>([]);
   
    const classes = useStyles();
    
    const getTabItem = (value, role: TaskUserRole, hidden: boolean) =>{
        const tab:ITab = {
            value,
            tabPanelChildren: <TaskUserTab role={role} task={props.task}
                 canAdd={props.canAdd} canDelete={props.canDelete} hideTitle />,
            tabProps: {
                icon: role === TaskUserRole.SUBMITOR ? <TouchApp /> : role === TaskUserRole.VALIDATOR ? <HowToReg /> :  <Person />,
                label: `${translate(`microgatewayApp.TaskUserRole.${role.toString()}`) + 's'}`,
                classes:{
                    root: `${classes.tab}`,
    ...
    const getTabs = () =>{
        ...
        if(serviceIsOnline(SetupService.PROCESS)){
            data.push(getTabItem(0,TaskUserRole.EXCEUTOR,false));
            // data.push(getTabItem(1,TaskUserRole.SUBMITOR,false));
            // data.push(getTabItem(2,TaskUserRole.VALIDATOR,false));
        }
        setTabs([...data])
    };
